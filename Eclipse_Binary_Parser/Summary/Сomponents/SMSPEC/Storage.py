...
class Storage:
    ...
    def get_position(self, name: str or None, keyword: str,
        ...
        if name is not None:
            name_pattern = self.__df[self.__target_name[0]] == name
        else:
            name_pattern = ~self.__df[self.__target_name[0]].isnull()
    ...
    def report(self):
        print(f'SMSPEC Keyword:\t{list(self.__df.keys())}')
        print(f'Field Object:\t'
              f'{list(pd.unique(self.__df[self.__target_name[0]]))}')
        print(f'Model Keyword (Mnemonic):\t'
              f'{list(pd.unique(self.__df[self.__target_name[1]]))}')
